...
class ArithmeticWorkflowTester:
    ...
    def run_single_test(
        ...
        try:
            with DAPClient() as client:
                # Initialize session
                client.initialize(
                    adapter_id="mlir-debugger", client_id=f"workflow-test-{test_name}"
                )
                ...
                for i in range(len(breakpoints)):
                    ...
                    test_result["dap_events"].append(
                        {"event": "breakpoint_hit", "breakpoint": i + 1, "result": result}
                    )
        ...
        for op in basic_ops:
            ...
            for i, inputs in enumerate(test_cases):
                ...
                self.run_single_test(
                    test_name=full_test_name, mlir_content=mlir_content, concrete_inputs=inputs
                )
    ...
    def run_complex_expression_tests(self):
        ...
        for i, ops in enumerate(complex_expressions):
            ...
            self.run_single_test(
                test_name=test_name, mlir_content=mlir_content, concrete_inputs={"a": 5, "b": 3}
            )
    ...
    def run_edge_case_tests(self):
        ...
        for op, inputs in edge_cases:
            ...
            self.run_single_test(
                test_name=test_name, mlir_content=mlir_content, concrete_inputs=inputs
            )
    ...
    def run_symbolic_tests(self):
        ...
        for op in symbolic_ops:
            ...
            self.run_single_test(
                test_name=test_name, mlir_content=mlir_content, symbolic=True, max_paths=3
            )
...
"""
    ...
    def generate_report(self):
        ...
        json_path = os.path.join(
            self.output_dir,
            f"arith_workflow_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
        )
        ...
        md_path = os.path.join(
            self.output_dir, f"arith_workflow_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
        with open(md_path, "w") as f:
            ...
            for result in self.test_results:
                ...
                f.write(
                    f"| {result['test_name']} | {status} | {result['execution_time']:.2f}s | {result.get('breakpoints_hit', 0)} | {result.get('paths_explored', 0)} |\n"
                )
...
def main():
    ...
    parser.add_argument("--basic", action="store_true", help="Run only basic arithmetic tests")
    parser.add_argument("--complex", action="store_true", help="Run only complex expression tests")
    parser.add_argument("--edge", action="store_true", help="Run only edge case tests")
    parser.add_argument("--symbolic", action="store_true", help="Run only symbolic execution tests")
    parser.add_argument("--concolic", action="store_true", help="Run only concolic tests")
