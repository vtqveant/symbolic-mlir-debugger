...
class DAPEvent:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result = {"seq": self.seq, "type": self.type.name.lower(), "event": self.event}
        if self.body:
            result["body"] = self.body
        return result
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "DAPEvent":
        ...
        return cls(event=data["event"], body=data.get("body"))
...
class LaunchRequest(DAPRequest):
    ...
    def __init__(self, program: str, no_debug: bool = True, **kwargs):
        arguments = {"program": program, "noDebug": no_debug, **kwargs}
...
class SetBreakpointsRequest(DAPRequest):
    ...
    def __init__(
        ...
        arguments = {"source": source, "breakpoints": breakpoints or []}
        if line_breakpoints:
