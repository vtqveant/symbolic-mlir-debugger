 ...
 *--------------------------------------------------------*/
/*
 * mlirDebug.ts implements the Debug Adapter that "adapts" or translates the Debug Adapter Protocol (DAP) used by the client (e.g. VS Code)
 * into requests and events of the MLIR debugger (stub implementation for inline mode).
 ...
 *
 * The most important class of the Debug Adapter is the MLIRDebugSession which implements many DAP requests (stubbed for external mode).
 */
...
type IRuntimeVariableType = number | boolean | string | RuntimeVariable[];
...
class RuntimeVariable {
    ...
    constructor(public readonly name: string, value: IRuntimeVariableType) {
        this._value = value;
    }
...
class MLIRRuntime extends EventEmitter {
    ...
    public stepOut(): void {}
    public getStepInTargets(frameId: number): any[] { return []; }
    public stack(startFrame: number, endFrame: number): any { return { count: 0, frames: [] }; }
    public getBreakpoints(path: string, line: number): number[] { return []; }
    public async setBreakPoint(path: string, line: number): Promise<IRuntimeBreakpoint> {
        return { id: 0, line, verified: true };
    }
    public clearBreakPoint(path: string, line: number): IRuntimeBreakpoint | undefined { return undefined; }
    public clearBreakpoints(path: string): void {}
    public setDataBreakpoint(address: string, accessType: 'read' | 'write' | 'readWrite'): boolean { return false; }
    public clearAllDataBreakpoints(): void {}
    public setExceptionsFilters(namedException: string | undefined, otherExceptions: boolean): void {}
    public setInstructionBreakpoint(address: number): boolean { return false; }
    public clearInstructionBreakpoints(): void {}
    public async getGlobalVariables(cancellationToken?: () => boolean): Promise<RuntimeVariable[]> { return []; }
    public getLocalVariables(): RuntimeVariable[] { return []; }
    public getLocalVariable(name: string): RuntimeVariable | undefined { return undefined; }
    public disassemble(address: number, instructionCount: number): any[] { return []; }
}

/**
 * This interface describes the MLIR debug specific launch attributes
 * (which are not part of the Debug Adapter Protocol).
 * The schema for these attributes lives in the package.json of the mlir-debug extension.
 * The interface should always match this schema.
 */
interface ILaunchRequestArguments extends DebugProtocol.LaunchRequestArguments {
	/** An absolute path to the MLIR file to debug. */
	program: string;
	/** Arguments to pass to the MLIR program (e.g., concrete inputs as key=value pairs). */
	args?: string[];
	...
	noDebug?: boolean;
	/** simulate a compile error; either 'show' (show error dialog) or 'hide' (send error but don't show dialog) */
	compileError?: string;
}
...
export class MLIRDebugSession extends LoggingDebugSession {

	// we don't support multiple threads, so we can use a hardcoded ID for the default thread
	private static threadID = 1;

	// MLIR runtime (stub for inline mode)
	private _runtime: MLIRRuntime;
	...
	public constructor(fileAccessor: FileAccessor) {
		super("mlir-debug.txt");

		// this debugger uses zero-based lines and columns
		this.setDebuggerLinesStartAt1(false);
		this.setDebuggerColumnsStartAt1(false);

		this._runtime = new MLIRRuntime(fileAccessor);

		// setup event handlers
		this._runtime.on('stopOnEntry', () => {
			this.sendEvent(new StoppedEvent('entry', MLIRDebugSession.threadID));
		});
		this._runtime.on('stopOnStep', () => {
			this.sendEvent(new StoppedEvent('step', MLIRDebugSession.threadID));
		});
		this._runtime.on('stopOnBreakpoint', () => {
			this.sendEvent(new StoppedEvent('breakpoint', MLIRDebugSession.threadID));
		});
		this._runtime.on('stopOnDataBreakpoint', () => {
			this.sendEvent(new StoppedEvent('data breakpoint', MLIRDebugSession.threadID));
		});
		this._runtime.on('stopOnInstructionBreakpoint', () => {
			this.sendEvent(new StoppedEvent('instruction breakpoint', MLIRDebugSession.threadID));
		});
		this._runtime.on('stopOnException', (exception) => {
			if (exception) {
				this.sendEvent(new StoppedEvent(`exception(${exception})`, MLIRDebugSession.threadID));
			} else {
				this.sendEvent(new StoppedEvent('exception', MLIRDebugSession.threadID));
			}
		});
	...
	protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
		...
		response.body = {
			threads: [
				new Thread(MLIRDebugSession.threadID, "thread 1"),
				new Thread(MLIRDebugSession.threadID + 1, "thread 2"),
			]
		};
		this.sendResponse(response);
	}
