...
class Parser(object):
    ...
    def __init__(self, dialects: Optional[List[Dialect]] = None):
        ...
        builtin_names = [dialect.name for dialect in UPSTREAM_DIALECTS]
        additional_names = [dialect.name for dialect in dialects]
        dialect_set = set(builtin_names) | set(additional_names)
        if len(dialect_set) != (len(dialects) + len(UPSTREAM_DIALECTS)):
            raise NameError(
        ...
        self.parser = Lark(parser_src, parser="earley", propagate_positions=True)
