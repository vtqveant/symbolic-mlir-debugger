...
class TraceExecutor:
    ...
    def execute_trace(self, trace_data: Dict[str, Any], 
                     trace_name: str = "unknown") -> Dict[str, Any]:
    ...
    def execute_directory(self, directory: Union[str, Path],
        ...
        for trace_file in trace_files:
            ...
            if self.statistics["executed_traces"] % 10 == 0:
                logger.info(f"Progress: {self.statistics['executed_traces']}/{self.statistics['total_traces']} traces executed")
        ...
        if self.statistics["executed_traces"] > 0:
            self.statistics["success_rate"] = (
                self.statistics["successful_traces"] / 
                self.statistics["executed_traces"] * 100
            )
            self.statistics["average_duration"] = (
                self.statistics["total_duration"] / 
                self.statistics["executed_traces"]
            )
        else:
    ...
    def execute_with_validation(self, trace_data: Dict[str, Any],
        ...
        for group_name, group_data in trace_groups.items():
            if group_data["count"] > 0:
                group_data["success_rate"] = (
                    group_data["successful"] / group_data["count"] * 100
                )
    ...
    def save_report(self, report: Dict[str, Any], 
        ...
        # Save as JSON
        json_path = output_dir / f"execution_report_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _save_markdown_report(self, report: Dict[str, Any], 
        ...
        with open(output_path, "w") as f:
            ...
            f.write(f"- **Minimum execution time**: {summary['execution_time']['minimum']:.2f}s\n\n")
            ...
            for group_name, group_data in trace_groups.items():
                f.write(f"| {group_name} | {group_data['count']} | {group_data['successful']} | "
