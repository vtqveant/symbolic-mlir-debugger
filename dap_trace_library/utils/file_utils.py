...
class FileUtils:
    ...
    @staticmethod
    def write_json(data: Dict[str, Any], path: Union[str, Path], 
                  indent: int = 2) -> Path:
    ...
    def copy_with_structure(source: Union[str, Path], 
                           target: Union[str, Path]) -> List[Path]:
    ...
    def clean_directory(directory: Union[str, Path],
                       keep_patterns: List[str] = None) -> int:
    ...
    def create_backup(source: Union[str, Path],
                     backup_dir: Union[str, Path] = None) -> Path:
    ...
    @staticmethod
    def merge_json_files(files: List[Union[str, Path]],
    ...
    @staticmethod
    def split_json_file(input_path: Union[str, Path],
        ...
        if isinstance(data, dict):
            ...
            chunk_size = max(1, len(items) // (FileUtils.get_file_size(input_path) // (max_size_kb * 1024) + 1))
            ...
            # Split list
            chunk_size = max(1, len(data) // (FileUtils.get_file_size(input_path) // (max_size_kb * 1024) + 1))
