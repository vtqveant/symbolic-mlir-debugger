...
class LoggingUtils:
    ...
    def setup_module_logging(module_name: str, level: int = logging.INFO) -> logging.Logger:
        ...
        if not logger.handlers:
            ...
            formatter = logging.Formatter(
                LoggingUtils.DEFAULT_FORMAT, LoggingUtils.DEFAULT_DATE_FORMAT
            )
    ...
    @staticmethod
    def create_structured_logger(name: str, log_file: Optional[Path] = None) -> logging.Logger:
    ...
    def log_with_context(
        logger: logging.Logger, level: int, message: str, context: Dict[str, Any] = None, **kwargs
    ) -> None:
    ...
    def log_performance(
        ...
        LoggingUtils.log_with_context(
            logger, logging.INFO, f"Performance: {operation} took {duration_seconds:.3f}s", context
        )
    ...
    def log_error_with_traceback(
        ...
        LoggingUtils.log_with_context(
            logger, logging.ERROR, f"Error: {type(error).__name__}: {str(error)}", error_context
        )

    @staticmethod
    def create_rotating_file_logger(
        name: str, log_dir: Path, max_bytes: int = 10 * 1024 * 1024, backup_count: int = 5  # 10 MB
    ) -> logging.Logger:
        ...
        # Create formatter
        formatter = logging.Formatter(LoggingUtils.DEFAULT_FORMAT, LoggingUtils.DEFAULT_DATE_FORMAT)
        handler.setFormatter(formatter)
    ...
    def setup_library_logging(log_dir: Optional[Path] = None) -> Dict[str, logging.Logger]:
        ...
        for module in modules:
            if log_dir:
                ...
                logger = LoggingUtils.create_rotating_file_logger(module, log_dir)
            else:
    ...
    def log_configuration(
        logger: logging.Logger, config: Dict[str, Any], config_name: str = "configuration"
    ) -> None:
        ...
        if "dialects" in config:
            ...
            enabled_operations = sum(
                len([op for op in d.get("operations", []) if op.get("enabled", True)])
