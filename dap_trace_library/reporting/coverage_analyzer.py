...
class CoverageAnalyzer:
    ...
    def analyze_generation_coverage(self, generation_results: Dict[str, Any]) -> None:
        ...
        for op in operations:
            ...
            if dialect not in self.dialect_coverage:
                self.dialect_coverage[dialect] = CoverageMetric(
                    coverage_type=CoverageType.DIALECT
                )
    ...
    def generate_coverage_report(self, output_dir: Union[str, Path] = None,
    ...
    def _generate_markdown_report(self, metrics: Dict[str, Any], output_path: Path) -> None:
        ...
        for dialect, metric in metrics['dialect_coverage'].items():
            lines.append(f"| {dialect} | {metric.coverage_percentage:.1f}% | "
        ...
        for op_key, op_coverage in metrics['operation_details'].items():
            if op_coverage.total_test_cases > 0:
                success_rate = (op_coverage.successful_test_cases / 
                              op_coverage.executed_test_cases * 100) if op_coverage.executed_test_cases > 0 else 0
                lines.append(f"| {op_coverage.operation_name} | {op_coverage.dialect} | "
