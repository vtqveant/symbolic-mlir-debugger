...
class ConfigurableGenerator(BaseGenerator):
        ...
        for dialect in self.config.get_enabled_dialects():
            ...
            templates[dialect_name] = {
                "mlir_template": dialect.generation_settings.get(
                    "mlir_template",
                    self._get_default_template(dialect)
                ),
                "trace_template": dialect.generation_settings.get(
                    "trace_template",
                    self._get_default_trace_template(dialect)
                ),
  ...
  func.func @test_{dialect}_{op_name}() {{
    ...
    def generate_mlir_snippet(self, operation: OperationConfig, 
        ...
        if not template:
            template = self._get_default_template(
                self.config.get_dialect(dialect_name)
            )
    ...
    def generate_dap_trace(self, operation: OperationConfig,
                          mlir_path: Path,
        ...
        if not trace_template:
            trace_template = self._get_default_trace_template(
                self.config.get_dialect(dialect_name)
            )
        ...
        if values:
            for var_name, var_value in values.items():
                trace["session"].insert(2, {  # Insert after setMode
    ...
    def _render_session_item(self, item: Dict[str, Any], 
    ...
    def get_concrete_values(self, operation: OperationConfig) -> List[Dict[str, Any]]:
        ...
        if (self.z3_generator.is_available() and 
            operation.constraints and 
            self.config.generation_settings.get("use_z3", True)):
        ...
        if operation.test_cases:
            logger.info(f"Using {len(operation.test_cases)} predefined test cases for {operation.name}")
