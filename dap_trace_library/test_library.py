...
from dap_trace_library.config.dialect_config import GeneratorConfig, DialectConfig, OperationConfig, DialectType, OperationCategory
...
class LibraryTester:
    ...
    def test_configuration(self) -> bool:
        ...
        try:
            ...
            print(f"✅ Config summary: {summary['enabled_dialects']} dialects, {summary['enabled_operations']} operations")
    ...
    def test_generation(self) -> bool:
        ...
        try:
            ...
            print(f"✅ Created generator with {len(config.get_enabled_operations())} enabled operations")
            ...
            if len(mlir_files) == stats["mlir_files_generated"]:
                ...
                print(f"❌ MLIR file count mismatch: expected {stats['mlir_files_generated']}, found {len(mlir_files)}")
                return False
            
            if len(trace_files) == stats["traces_generated"]:
                print(f"✅ DAP trace files created: {len(trace_files)}")
            else:
                print(f"❌ DAP trace file count mismatch: expected {stats['traces_generated']}, found {len(trace_files)}")
                return False
    ...
    def test_validation(self) -> bool:
        ...
        try:
            ...
            if trace_files := list(trace_dir.glob("*.json")):
                ...
                print(f"✅ Single file validation: {'VALID' if single_result['valid'] else 'INVALID'}")
    ...
    def test_integration(self) -> bool:
        ...
        try:
            ...
            print(f"   MLIR: {mlir_validation['files_valid']}/{mlir_validation['files_validated']} valid")
            print(f"   Traces: {trace_validation['files_valid']}/{trace_validation['files_validated']} valid")
            
            # Check manifest was created
            manifest_dir = Path(config.output_settings["base_dir"]) / config.output_settings["manifest_dir"]
            ...
            if manifest_files:
                ...
                if "tests" in manifest and "dialects" in manifest:
                    print(f"✅ Manifest structure correct: {len(manifest['tests'])} tests, {len(manifest['dialects'])} dialects")
                else:
            ...
            return (mlir_validation["valid"] and 
                   trace_validation["valid"] and 
                   generation_result["statistics"]["mlir_files_generated"] > 0)
