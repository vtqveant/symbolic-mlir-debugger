...
def generate_test_report(validation_results: List[Dict], execution_results: List[Dict]) -> str:
    ...
    report_lines.extend([
        ...
        f"- **Format Success Rate:** {valid_tests/total_tests:.1%}" if total_tests > 0 else "- **Format Success Rate:** N/A",
        f"- **Successful Simulations:** {successful_simulations}",
        f"- **Simulation Success Rate:** {successful_simulations/total_tests:.1%}" if total_tests > 0 else "- **Simulation Success Rate:** N/A",
        "",
        "## Test Type Breakdown",
        ""
    ])
    ...
    for i, (val_result, exec_result) in enumerate(zip(validation_results, execution_results)):
        ...
        report_lines.append(f"- **Format Valid:** {'✅ Yes' if val_result['valid_format'] else '❌ No'}")
        report_lines.append(f"- **Test Type:** {val_result['test_structure'].get('type', 'unknown').replace('_', ' ').title()}")
        report_lines.append(f"- **Session Steps:** {val_result['test_structure'].get('session_steps', 0)}")
        ...
        report_lines.append(f"- **Simulation:** {'✅ Success' if exec_result['simulation_success'] else '❌ Failed'}")
    ...
    coverage_stats = {
        ...
        "conditional_branches": sum(1 for r in validation_results if "conditional" in r["name"].lower()),
    ...
    if valid_tests == total_tests:
        report_lines.append("✅ **Excellent test format quality:** All tests have valid JSON structure")
    else:
...
def main():
    ...
    for test_file in test_files:
        ...
        print(f"  {status} Format: {len(val_result['errors'])} errors, {len(val_result['warnings'])} warnings")
