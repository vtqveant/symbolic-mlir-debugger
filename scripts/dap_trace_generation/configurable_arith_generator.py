...
class ConfigurableArithGenerator:
    ...
    def generate_mlir_artifacts(self) -> None:
        ...
        for op_config in self.config['operations']:
            ...
            elif op_name in ['extsi', 'extui', 'trunci', 'sitofp', 'uitofp', 'fptosi', 'fptoui']:
        ...
        print(f"Generated {self.stats['mlir_files_generated']} MLIR files for {self.stats['enabled_operations']} operations")
...
"""
    ...
    def _validate_mlir_file(self, filename: Path) -> bool:
        ...
        try:
            ...
            validate_script = self.base_dir / "scripts" / "validate_mlir_precommit.py"
            if validate_script.exists():
                result = subprocess.run(
                    [sys.executable, str(validate_script), str(filename)],
                    capture_output=True,
                    text=True,
                    timeout=self.config['generation_settings'].get('validation_timeout_ms', 10000) / 1000
                )
    ...
    def _generate_trace_for_mlir(self, mlir_file: Path) -> None:
        ...
        try:
            ...
            # Generate test cases
            test_cases = generator.generate_from_file(str(mlir_file))
    ...
    def _validate_dap_trace(self, trace_file: Path, mlir_file: Path) -> bool:
        ...
        try:
            ...
            # Execute test cases
            results = runner.run_tests(trace_data['test_cases'], str(mlir_file))
    ...
    def _generate_coverage_report(self) -> str:
        ...
        disabled_ops = [op['name'] for op in self.config['operations'] if not op.get('enabled', False)]
...
"""
    ...
    def _get_config_summary(self) -> str:
        ...
        summary = {
            ...
            "enabled_operations_count": sum(1 for op in self.config['operations'] if op.get('enabled', False)),
...
def main():
    ...
    parser.add_argument(
        "--mlir-dir",
        help="Custom MLIR artifacts directory (overrides config)"
    )
    parser.add_argument(
        "--trace-dir",
        help="Custom DAP traces directory (overrides config)"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Dry run (no file generation)"
    )
