...
def test_config_loading():
    ...
    try:
        ...
        print(
            f"  - Enabled operations: {sum(1 for op in config['operations'] if op.get('enabled', False))}"
        )
...
def test_generator_import():
    ...
    try:
        from scripts.dap_trace_generation.configurable_arith_generator import (
            ConfigurableArithGenerator,
        )
...
def test_mlir_generation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Copy config file
        config_src = Path("target/trace_testing/arith_ops_config.yaml")
