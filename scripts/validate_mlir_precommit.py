...
def validate_python_file(file_path: str) -> list:
    ...
    for i, block in enumerate(mlir_blocks):
        uri = f"file://{os.path.abspath(file_path)}#block_{i+1}_line_{block['line_start']}"
...
def main():
    ...
    if args.staged:
        ...
        mlir_files = [str(f) for f in mlir_files if ".venv" not in str(f) and "__pycache__" not in str(f)]
    ...
    files_to_validate = [
        ...
        if f.endswith('.mlir') or f.endswith('.py')
    ]
    ...
    if problems:
        ...
        print("=" * 80)
        print(f"\nSummary: Validated {mlir_count} .mlir files and {python_count} Python files")
